...
def weight_distribution(pfi_model, **kwargs):    
    if kwargs:
...
class DatasetSampling(object):
    def __init__(self,dataset,num_images_x_class, num_of_class):   

        self.length=len(dataset)
        self.num_images=num_images_x_class*num_of_class
        self.num_of_class=num_of_class
        self.num_images_x_class = num_images_x_class
        self.image_for_classes = int(int(self.length/1000))
        self.indices=[]

    def listindex(self):
        self.indices=[]
        for i in range(0,int(self.length*(self.num_of_class/1000)), ):
            if (i%self.image_for_classes)< self.num_images_x_class:
